...
class TopModuleComVis(val imgWidth: Int, val TPN: Int, val symbolN: Int, val templateStringName: String)
    extends Module {
  ...
  val templateFiles = (0 until TPN * symbolN).map(i => "templates/" + templateStringName + s"_${i}.hex")
