 ...
 *
 * Top-level module combining decoder and TDM controller.
 * Designed for Nexys A7 board with 8-digit common-anode display.
 *
 * @param maxScore Theoretical maximum score (computed at elaboration)
 * @param refreshDiv Clock divider for TDM refresh rate
 */
...
object SevenSegDriver {
  ...
  /** Create driver for standard 32x32 MNIST with 10 templates per digit
   * maxScore = 32 × 32 × 10 = 10,240
   * Requires 14-bit confidence input
   */
